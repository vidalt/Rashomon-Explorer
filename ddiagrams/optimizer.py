...
class Optimizer:
    ...
    def __init__(self, data, topology, metric = "SP", reverse = False, alpha=0.0, initial_solution=None, test = None, univariate_split=False, sym_break=True,
                 force_tree=False, fair=False, use_previous=False, max_nodes=1000, translate_loss=1):
        ...
        with gp.Model() as model:
            ...
            if use_previous:
                for i, s in enumerate(topology.arcs):
                    y[s].Start = test.y[str(i)]
                for i, s in enumerate(sample_terminal_pairs):
                    w_t[s].Start = test.w_t[str(i)]
                for i, s in enumerate(sample_nonterminal_pairs):
                    w_p[s].Start = test.w_p[str(i)]
                for i, s in enumerate(sample_nonterminal_pairs):
                    w_n[s].Start = test.w_n[str(i)]
                for i, s in enumerate(sample_layer_pairs):
                    lambda_var[s].Start = test.lambda_var[str(i)]
            a = {}
            ap = {}
            an = {}
            b = {}
            for u in topology.nonterminal_nodes:
                ...
                if univariate_split:
                    ...
                    model.addConstr(gp.quicksum(ap[u][f]+an[u][f] for f in data.features)==1)
                else:
            ...
            for (i, v) in sample_nonterminal_pairs:
                if v == topology.root_node:
                    ...
                    model.addConstr(w_p[(i, v)] + w_n[(i, v)] == gp.quicksum(z[(i, s, u, v)]
                            for (s, u, vv) in topology.arcs_arriving_at_node[v]))

            # constraints (2) and (3)
            for (i, u) in sample_nonterminal_pairs:
                model.addConstr(w_n[(i, u)] == gp.quicksum(z[(i, "-", u, v)]
                        for (s, uu, v) in topology.neg_arcs_departing_node[u]))
                model.addConstr(w_p[(i, u)] == gp.quicksum(z[(i, "+", u, v)]
            ...
            for (i, l) in sample_layer_pairs:
                if l == topology.layers - 1:
                    continue
                model.addConstr(gp.quicksum(w_n[(i, u)] for u in topology.nodes_per_layer[l])
                        <= 1 - lambda_var[(i, l)])
                model.addConstr(gp.quicksum(w_p[(i, u)] for u in topology.nodes_per_layer[l])
                        <= lambda_var[(i, l)])

            # constraint (6)
            for l in range(topology.layers - 1):
                for u in topology.nodes_per_layer[l]:
                    model.addConstr(d[u] == gp.quicksum(y[arc]
            ...
            for l in range(1, topology.layers - 1):
                for v in topology.nodes_per_layer[l]:
                    model.addConstr(d[v] <= gp.quicksum(y[arc]
                ...
                for v in topology.nodes_per_layer[l]:
                    for i in range(data.train_n):
                        model.addConstr((w_n[(i, v)] == 1) >>
                                (gp.quicksum(a[v][f]*data.train_X[i][f]
            ...
            # constraint (17)
            for (i, v) in sample_terminal_pairs:
                model.addConstr(w_t[(i, v)] == gp.quicksum(z[(i, s, u, v)]
                        for (s, u, vv) in topology.arcs_arriving_at_node[v]))
            ...
            if not fair:
                ...
                var = gp.quicksum(w_t[(i, v)]*assignment_cost[(i, v)]
                    for (i, v) in sample_terminal_pairs)/data.train_n + alpha*gp.quicksum(d[v] 
                    for v in topology.internal_nodes)/len(topology.internal_nodes)
                model.setObjective(var)
            else:
                m = [(data.train_Y[i] == 1) for i in range (len(data.train_Y))]
                o = [(data.train_Y[i] == 0) for i in range (len(data.train_Y))]
                SP_index = []
                n_SP_index = []
                EO_index = []
                n_EO_index = []
                PE_index = []
                n_PE_index = []
                for i, index in enumerate(data.Xt.index):
                    if data.Xt[data.protected_class][index] == 1:
                        SP_index.append(i)
                        if data.Yt[i] == 0:
                            PE_index.append(i)
                        else:
                ...
                var = gp.quicksum(w_t[(i, v)]*assignment_cost[(i, v)]
                    for (i, v) in sample_terminal_pairs)/data.train_n
                model.addConstr(var <= translate_loss)
                model.addConstr(gp.quicksum(d[v] for v in topology.internal_nodes) <= max_nodes)
                Z = []
                for i in range (len(data.train_Y)):
                    t = 0
                    for v in topology.nodes_per_layer[topology.layers-1]:
                        t += w_t[(i, v)]*assignment_cost[(i, v)]
                    Z.append(t)

                fairness = (len(SP_index) * gp.quicksum((1 - Z[i]) * m[i] + o[i] * Z[i] for i in n_SP_index) - (len(n_SP_index)) * gp.quicksum((1 - Z[i]) * m[i] + o[i] * Z[i] for i in SP_index))
                model.setObjective(fairness / (len(SP_index) * len(n_SP_index)))

                fairness = 0
                if metric == "SP":
                    fairness = (len(SP_index) * gp.quicksum((1 - Z[i]) * m[i] + o[i] * Z[i] for i in n_SP_index) - (len(n_SP_index)) * gp.quicksum((1 - Z[i]) * m[i] + o[i] * Z[i] for i in SP_index))
                    fairness = fairness / (len(SP_index) * len(n_SP_index))
                elif metric == "PE":
                    fairness = len(PE_index) * gp.quicksum((1 - Z[i]) * m[i] + o[i] * Z[i] for i in n_PE_index) - len(n_PE_index) * gp.quicksum((1 - Z[i]) * m[i] + o[i] * Z[i] for i in PE_index)
                    fairness = fairness / (len(PE_index) * len(n_PE_index))
                elif metric == "EO":
                    fairness = len(EO_index) * gp.quicksum((1 - Z[i]) * m[i] + o[i] * Z[i] for i in n_EO_index) - len(n_EO_index) * gp.quicksum((1 - Z[i]) * m[i] + o[i] * Z[i] for i in EO_index)
                    fairness = fairness / (len(EO_index) * len(n_EO_index))

                if reverse:
                    model.setObjective(-fairness, GRB.MINIMIZE)
                else:
                    model.setObjective(fairness, GRB.MINIMIZE)
            ...
            if sym_break:
                ...
                for l in range(topology.layers - 1):
                    for u in topology.nodes_per_layer[l]:
                        for (s, uu, v) in topology.pos_arcs_departing_node[u]:
                            assert s == "+" and uu == u
                            model.addConstr(y[("-", u, v)] + gp.quicksum(y[("+", u, w)] for
                ...
                for l in range(1, topology.layers - 1):
                    for u in topology.nodes_per_layer[l]:
                        for v in topology.nodes_per_layer[l]:
                            if u < v:
                                model.addConstr(gp.quicksum(y[arc]
            ...
            for l in range(1, topology.layers - 1):
                for v in topology.nodes_per_layer[l]:
                    model.addConstr(gp.quicksum(z[(i, s, u, v)]
                            for (s, u, vv) in topology.arcs_arriving_at_node[v]
            ...
            if model.SolCount > 0:
                ...
                for v in topology.nodes:
                    ...
                    if d[v].X >= 0.9:
                        ...
                        self.solution.nb_active_nodes += 1
                        if cfg.verbose:
                ...
                for i, s in enumerate(topology.arcs):
                    self.solution.y[i] = y[s].X
                for i, s in enumerate(sample_terminal_pairs):
                    self.solution.w_t[i] = w_t[s].X
                for i, s in enumerate(sample_nonterminal_pairs):
                    self.solution.w_p[i] = w_p[s].X
                for i, s in enumerate(sample_nonterminal_pairs):
                    self.solution.w_n[i] = w_n[s].X
                for i, s in enumerate(sample_layer_pairs):
                    self.solution.lambda_var[i] = lambda_var[s].X
                self.solution.status = "known_sol"
                ...
                if model.status == GRB.INFEASIBLE:
                    ...
                    self.solution.status = "no_known_sol"